...
public class SparkTypeProvider extends AbstractBeelineJdbcTypeProvider<SparkConnectionProperties> {

    private static final Set<String> VALID_AUTH = Set.of("NONE", "KERBEROS");
    ...
    @Override
    public Class<SparkConnectionProperties> propertiesClass() {
        return SparkConnectionProperties.class;
    }
    ...
    @Override
    public ValidationResult validate(SparkConnectionProperties props) {
        if (props == null) {
        ...
        if ("KERBEROS".equals(props.auth())) {
            if (props.principal() == null || props.principal().isBlank()) {
                return ValidationResult.fail("principal", "principal is required when auth=KERBEROS");
            }
            if (props.keytabPath() == null || props.keytabPath().isBlank()) {
