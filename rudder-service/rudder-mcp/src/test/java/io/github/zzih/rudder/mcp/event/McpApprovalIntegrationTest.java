...
class McpApprovalIntegrationTest {
    ...
    @InjectMocks
    private McpApprovalIntegration integration;
    ...
    @Test
    @DisplayName("resourceType 返回 MCP_TOKEN")
    void resourceTypeIsMcpToken() {
        assertThat(integration.resourceType()).isEqualTo(ApprovalResourceType.MCP_TOKEN);
    }

    @Test
    @DisplayName("buildRequest 默认抛 UnsupportedOperationException (MCP 提交走直调,非 facade)")
    void buildRequestUnsupported() {
        org.assertj.core.api.Assertions.assertThatThrownBy(() -> integration.buildRequest(1L))
    ...
    void approvedActivatesAllGrants() {
        ...
        integration.onFinalized(new ApprovalFinalizedEvent(
