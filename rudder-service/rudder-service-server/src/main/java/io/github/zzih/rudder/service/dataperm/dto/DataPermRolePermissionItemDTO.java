...
/**
 * 资源包内权限项 DTO,跨 controller / service 边界。
 *
 * <p>资源定位 4 元组 + accesses 列表。catalog/db/table/column 任一可为 NULL("此层不适用此 plugin")
 * 或 {@code "*"}("该层级全部")。
 */
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class DataPermRolePermissionItemDTO {

    /** 数据库主键。page 返回时填,创建入参时为 null。 */
    private Long id;
    ...
    /** plugin 原生 access 字符串列表(如 ["select","insert"])。 */
