...
public interface PublishRecordMapper extends BaseMapper<PublishRecord> {
    ...
    List<PublishRecord> selectByBatchCode(@Param("batchCode") Long batchCode);

    IPage<PublishBatchRow> queryBatchPage(IPage<?> page,
                                          @Param("projectCode") Long projectCode,
                                          @Param("status") String status);

    List<PublishBatchDetailRow> queryDetailsByBatchCodes(@Param("batchCodes") List<Long> batchCodes);
}
