...
public interface DataPermUserBundleGrantMapper extends BaseMapper<DataPermUserBundleGrant> {
    ...
    /** 查 user 在某时间点处于"有效"状态的所有 role grants;LEFT JOIN role 出 bundleName。 */
    List<DataPermUserBundleGrantDetailView> queryActiveByUser(@Param("userId") Long userId,
                                                              @Param("asOf") LocalDateTime asOf);

    /** 批量版本,reconciler 一次拉全部 active user 的 role grants。 */
    List<DataPermUserBundleGrant> queryActiveByUserIds(@Param("userIds") java.util.Collection<Long> userIds,
                                                       @Param("asOf") LocalDateTime asOf);

    /** "我的权限" 列表骨架:按 bundleId 聚合,LEFT JOIN role 名,子查询 perm_count。1 次 SQL 全出。 */
    ...
    List<DataPermUserBundleGrant> queryByApprovalId(@Param("approvalId") Long approvalId);
    ...
    int expireAllByBundleId(@Param("bundleId") Long bundleId,
                            @Param("now") LocalDateTime now,
                            @Param("endReason") String endReason);
